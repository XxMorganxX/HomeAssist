...
TRAINING_EXAMPLES = [
    ...
    ("Show my to-do list", [{"name": "todos", "arguments": {"action": "list"}}]),
    ("What's on my to-do list?", [{"name": "todos", "arguments": {"action": "list"}}]),
    ("My to-do list", [{"name": "todos", "arguments": {"action": "list"}}]),
    ("Read my to-dos", [{"name": "todos", "arguments": {"action": "list"}}]),
    ("What tasks do I have?", [{"name": "todos", "arguments": {"action": "list"}}]),
    ("Show me my tasks", [{"name": "todos", "arguments": {"action": "list"}}]),
    ...
    # --- Add to-do items ---
    ("Add buy milk to my to-do", [{"name": "todos", "arguments": {"action": "create", "title": "buy milk"}}]),
    ("Add call mom to the list", [{"name": "todos", "arguments": {"action": "create", "title": "call mom"}}]),
    ("Put pick up dry cleaning on my to-do", [{"name": "todos", "arguments": {"action": "create", "title": "pick up dry cleaning"}}]),
    ("Add grocery shopping to my list", [{"name": "todos", "arguments": {"action": "create", "title": "grocery shopping"}}]),
    ("Remind me to take out the trash", [{"name": "todos", "arguments": {"action": "create", "title": "take out the trash"}}]),
    ("Add send email to boss", [{"name": "todos", "arguments": {"action": "create", "title": "send email to boss"}}]),
    ("Put pay bills on my to-do list", [{"name": "todos", "arguments": {"action": "create", "title": "pay bills"}}]),
    ("Add finish report to my tasks", [{"name": "todos", "arguments": {"action": "create", "title": "finish report"}}]),
    ("Add walk the dog to my list", [{"name": "todos", "arguments": {"action": "create", "title": "walk the dog"}}]),
    ("Add schedule dentist appointment", [{"name": "todos", "arguments": {"action": "create", "title": "schedule dentist appointment"}}]),
    
    # --- Add multiple to-do items ---
    ("Add eggs, milk, and bread to my list", [{"name": "todos", "arguments": {"action": "create", "title": "eggs"}}, {"name": "todos", "arguments": {"action": "create", "title": "milk"}}, {"name": "todos", "arguments": {"action": "create", "title": "bread"}}]),
    ("Add these to my to-do: call mom, pay rent, buy groceries", [{"name": "todos", "arguments": {"action": "create", "title": "call mom"}}, {"name": "todos", "arguments": {"action": "create", "title": "pay rent"}}, {"name": "todos", "arguments": {"action": "create", "title": "buy groceries"}}]),
    ("Put apples and oranges on my list", [{"name": "todos", "arguments": {"action": "create", "title": "apples"}}, {"name": "todos", "arguments": {"action": "create", "title": "oranges"}}]),
    
    # --- Remove to-do items ---
    ("Remove buy milk from my list", [{"name": "todos", "arguments": {"action": "delete", "match": "buy milk"}}]),
    ("Cross off call mom", [{"name": "todos", "arguments": {"action": "complete", "match": "call mom"}}]),
    ("Delete grocery shopping from my to-do", [{"name": "todos", "arguments": {"action": "delete", "match": "grocery shopping"}}]),
    ("I finished the report, remove it", [{"name": "todos", "arguments": {"action": "complete", "match": "report"}}]),
    ("Done with taking out the trash", [{"name": "todos", "arguments": {"action": "complete", "match": "trash"}}]),
    ("Check off pay bills", [{"name": "todos", "arguments": {"action": "complete", "match": "pay bills"}}]),
    
    # --- Edit to-do items ---
    ("Change call mom to call dad", [{"name": "todos", "arguments": {"action": "update", "match": "call mom", "title": "call dad"}}]),
    ("Update buy milk to buy almond milk", [{"name": "todos", "arguments": {"action": "update", "match": "buy milk", "title": "buy almond milk"}}]),
    ...
    ("Add finish report by Friday to my list", [{"name": "todos", "arguments": {"action": "create", "title": "finish report", "due_at": "Friday"}}]),
    ("Add call landlord by Monday", [{"name": "todos", "arguments": {"action": "create", "title": "call landlord", "due_at": "Monday"}}]),
    ("Add submit application due Feb 15", [{"name": "todos", "arguments": {"action": "create", "title": "submit application", "due_at": "Feb 15"}}]),
    ...
    # =========================================================================
    # TODOS TOOL - persistent tasks and user-driven reminders
    # Params: action (required), title/message, due_at, event_time,
    #         remind_before_minutes, todo_id, match
    # =========================================================================

    # --- Create todos / reminder-style tasks ---
    ("Remind me at 9am to take my medication", [{"name": "todos", "arguments": {"action": "create", "title": "Take your medication", "due_at": "9am"}}]),
    ("Set a reminder for 3pm about the meeting", [{"name": "todos", "arguments": {"action": "create", "title": "You have a meeting", "due_at": "3pm"}}]),
    ("Remind me tomorrow at 8am to call mom", [{"name": "todos", "arguments": {"action": "create", "title": "Call mom", "due_at": "tomorrow 8am"}}]),
    ("Create a reminder for 5pm to leave for dinner", [{"name": "todos", "arguments": {"action": "create", "title": "Time to leave for dinner", "due_at": "5pm"}}]),
    ("Remind me at noon to eat lunch", [{"name": "todos", "arguments": {"action": "create", "title": "Time for lunch", "due_at": "12pm"}}]),
    ("Set a reminder for 7am about the gym", [{"name": "todos", "arguments": {"action": "create", "title": "Time for the gym", "due_at": "7am"}}]),
    ("Remind me at 10pm to go to bed", [{"name": "todos", "arguments": {"action": "create", "title": "Time for bed", "due_at": "10pm"}}]),
    ("Create a reminder at 2pm to check email", [{"name": "todos", "arguments": {"action": "create", "title": "Check your email", "due_at": "2pm"}}]),

    # --- Event-relative reminder-style todos ---
    ("Remind me 30 minutes before my 3pm meeting", [{"name": "todos", "arguments": {"action": "create", "title": "Meeting in 30 minutes", "event_time": "3pm", "remind_before_minutes": 30}}]),
    ("Remind me 15 minutes before the dentist at 2pm", [{"name": "todos", "arguments": {"action": "create", "title": "Dentist appointment in 15 minutes", "event_time": "2pm", "remind_before_minutes": 15}}]),
    ("Give me a 1 hour heads up before my 5pm dinner", [{"name": "todos", "arguments": {"action": "create", "title": "Dinner in 1 hour", "event_time": "5pm", "remind_before_minutes": 60}}]),

    # --- List todos / reminders ---
    ("What reminders do I have?", [{"name": "todos", "arguments": {"action": "list"}}]),
    ("Show my to-do list", [{"name": "todos", "arguments": {"action": "list"}}]),
    ("List my reminders", [{"name": "todos", "arguments": {"action": "list"}}]),
    ("What tasks are pending?", [{"name": "todos", "arguments": {"action": "list"}}]),
    ("Any pending reminders?", [{"name": "todos", "arguments": {"action": "list"}}]),
    ("What's due today?", [{"name": "todos", "arguments": {"action": "list", "only_due_today": True}}]),
    ("What's overdue?", [{"name": "todos", "arguments": {"action": "list", "only_overdue": True}}]),

    # --- Complete / delete todos ---
    ("Cancel the medication reminder", [{"name": "todos", "arguments": {"action": "delete", "match": "medication"}}]),
    ("Delete that reminder", [{"name": "todos", "arguments": {"action": "delete"}}]),
    ("Cancel my reminders", [{"name": "todos", "arguments": {"action": "delete"}}]),
    ("Remove the meeting reminder", [{"name": "todos", "arguments": {"action": "delete", "match": "meeting"}}]),
    ("Mark buy milk done", [{"name": "todos", "arguments": {"action": "complete", "match": "buy milk"}}]),
    ("Reopen call mom", [{"name": "todos", "arguments": {"action": "reopen", "match": "call mom"}}]),

    # --- Briefings remain autonomous/read-only ---
    ...
    # ---- TODOS (task + reminder timing) ----
    ("Add eggs to my list and remind me at 5pm to go shopping", [
        {"name": "todos", "arguments": {"action": "create", "title": "eggs"}},
        {"name": "todos", "arguments": {"action": "create", "title": "Time to go shopping", "due_at": "5pm"}}
    ]),
    ("Add milk to my to-do and set a reminder for the grocery store", [
        {"name": "todos", "arguments": {"action": "create", "title": "milk"}},
        {"name": "todos", "arguments": {"action": "create", "title": "Don't forget to buy milk", "due_at": "4pm"}}
    ]),
    ("Add take medicine to my list and remind me at 9am", [
        {"name": "todos", "arguments": {"action": "create", "title": "take medicine"}},
        {"name": "todos", "arguments": {"action": "create", "title": "Take your medicine", "due_at": "9am"}}
    ]),
    ("Add submit report and set a reminder for noon", [
        {"name": "todos", "arguments": {"action": "create", "title": "submit report"}},
        {"name": "todos", "arguments": {"action": "create", "title": "Submit your report", "due_at": "12pm"}}
    ]),
    ("Put laundry on my list and remind me at 6pm", [
        {"name": "todos", "arguments": {"action": "create", "title": "laundry"}},
        {"name": "todos", "arguments": {"action": "create", "title": "Do the laundry", "due_at": "6pm"}}
    ]),
    
    # ---- STICKIES + SMS (task + text reminder) ----
    ("Add buy groceries to my list and text me about it", [
        {"name": "todos", "arguments": {"action": "create", "title": "buy groceries"}},
        {"name": "send_sms", "arguments": {"message": "Reminder: Buy groceries"}}
    ]),
    ("Add call dentist to my to-do and send me a text reminder", [
        {"name": "todos", "arguments": {"action": "create", "title": "call dentist"}},
        {"name": "send_sms", "arguments": {"message": "Reminder: Call the dentist"}}
    ]),
    
    # ---- CALENDAR + TODOS (event + linked task view) ----
    ("Schedule a meeting at 3pm and add it to my to-do list", [
        ...
        {"name": "todos", "arguments": {"action": "create", "title": "meeting at 3pm", "source_type": "calendar"}}
    ]),
    ("Add dentist to my calendar tomorrow at 2pm and my to-do list", [
        ...
        {"name": "todos", "arguments": {"action": "create", "title": "dentist appointment tomorrow", "source_type": "calendar"}}
    ]),
    ("Schedule gym at 7am and add it to my list", [
        ...
        {"name": "todos", "arguments": {"action": "create", "title": "gym at 7am", "source_type": "calendar"}}
    ]),
    ("Add lunch with John at noon to both my calendar and to-do", [
        ...
        {"name": "todos", "arguments": {"action": "create", "title": "lunch with John at noon", "source_type": "calendar"}}
    ]),
    
    # ---- CALENDAR + TODOS (event + reminder task) ----
    ("Schedule a meeting at 3pm and remind me 30 minutes before", [
        {"name": "calendar_data", "arguments": {"commands": [{"read_or_write": "create_event", "event_title": "Meeting", "start_time": "3pm"}]}},
        {"name": "todos", "arguments": {"action": "create", "title": "Meeting in 30 minutes", "event_time": "3pm", "remind_before_minutes": 30}}
    ]),
    ("Add dentist at 2pm and set a reminder for 1:30pm", [
        ...
        {"name": "todos", "arguments": {"action": "create", "title": "Dentist appointment soon", "due_at": "1:30pm"}}
    ]),
    ("Schedule dinner at 7pm and remind me an hour before", [
        ...
        {"name": "todos", "arguments": {"action": "create", "title": "Dinner in 1 hour", "event_time": "7pm", "remind_before_minutes": 60}}
    ]),
    ...
    # ---- SEARCH + TODOS (find info + save task) ----
    ("Google the best sushi places and add 'find sushi restaurant' to my to-do", [
        ...
        {"name": "todos", "arguments": {"action": "create", "title": "find sushi restaurant"}}
    ]),
    ...
    # ---- TODOS + TODOS (read + write in one) ----
    ("Show my to-do list and add buy milk", [
        {"name": "todos", "arguments": {"action": "list"}},
        {"name": "todos", "arguments": {"action": "create", "title": "buy milk"}}
    ]),
    
    # ---- CALENDAR + TODOS (triple combo) ----
    ("Schedule dentist at 2pm, add it to my list, and remind me 1 hour before", [
        {"name": "calendar_data", "arguments": {"commands": [{"read_or_write": "create_event", "event_title": "Dentist", "start_time": "2pm"}]}},
        {"name": "todos", "arguments": {"action": "create", "title": "dentist at 2pm", "source_type": "calendar"}},
        {"name": "todos", "arguments": {"action": "create", "title": "Dentist in 1 hour", "event_time": "2pm", "remind_before_minutes": 60}}
    ]),
    ...
    ("Cooking time - search for pasta recipe and add groceries to my list", [
        ...
        {"name": "todos", "arguments": {"action": "create", "title": "buy groceries for pasta"}}
    ]),

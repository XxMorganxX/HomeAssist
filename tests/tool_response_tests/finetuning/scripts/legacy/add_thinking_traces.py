...
def think_briefing(args, user_prompt):
    ...
    if action == "list":
        lines.append(f"{pick(USER_WANTS)} to see pending autonomous briefings.")
...
def think_todos(args, user_prompt):
    action = args.get("action", "list")
    lines = []

    if action == "create":
        title = args.get("title") or args.get("message", "")
        due_at = args.get("due_at")
        event_time = args.get("event_time")
        before_min = args.get("remind_before_minutes")
        if due_at:
            lines.append(f"{pick(USER_WANTS)} a persistent reminder/task due at {due_at}: \"{title}\".")
            lines.append(f"{pick(I_SHOULD)} todos with action=\"create\", due_at=\"{due_at}\".")
        elif event_time and before_min:
            lines.append(f"{pick(USER_WANTS)} a task reminder {before_min} minutes before {event_time}: \"{title}\".")
            lines.append(f"{pick(I_SHOULD)} todos with action=\"create\", event_time=\"{event_time}\", remind_before_minutes={before_min}.")
        else:
            lines.append(f"{pick(USER_WANTS)} to add a todo: \"{title}\".")
            lines.append(f"{pick(I_SHOULD)} todos with action=\"create\".")
    elif action == "list":
        if args.get("only_due_today"):
            lines.append(f"{pick(USER_WANTS)} to see todos due today.")
            lines.append(f"{pick(I_SHOULD)} todos with action=\"list\" and only_due_today=true.")
        elif args.get("only_overdue"):
            lines.append(f"{pick(USER_WANTS)} to see overdue todos.")
            lines.append(f"{pick(I_SHOULD)} todos with action=\"list\" and only_overdue=true.")
        else:
            lines.append(f"{pick(USER_WANTS)} to see their persistent todos/reminders.")
            lines.append(f"{pick(I_SHOULD)} todos with action=\"list\".")
    elif action == "complete":
        lines.append(f"{pick(USER_WANTS)} to mark a todo complete.")
        lines.append(f"{pick(I_SHOULD)} todos with action=\"complete\".")
    elif action == "reopen":
        lines.append(f"{pick(USER_WANTS)} to reopen a completed todo.")
        lines.append(f"{pick(I_SHOULD)} todos with action=\"reopen\".")
    elif action == "update":
        lines.append(f"{pick(USER_WANTS)} to edit an existing todo.")
        lines.append(f"{pick(I_SHOULD)} todos with action=\"update\".")
    elif action == "delete":
        lines.append(f"{pick(USER_WANTS)} to delete a todo/reminder.")
        lines.append(f"{pick(I_SHOULD)} todos with action=\"delete\".")
...
def generate_thinking_trace(user_prompt: str, tool_calls: list) -> str:
    ...
    for tc in tool_calls:
        ...
        if name == "weather":
            ...
            action = args.get("action", "list")
            if action == "list":
                parts.append(f"List autonomous briefings → briefing.")
            else:
                parts.append(f"Dismiss briefing → briefing.")
        elif name == "todos":
            action = args.get("action", "list")
            if action == "create":
                parts.append("Create persistent reminder/task → todos.")
            elif action == "list":
                parts.append("List todos/reminders → todos.")
            elif action == "complete":
                parts.append("Complete todo → todos.")
            elif action == "reopen":
                parts.append("Reopen todo → todos.")
            elif action == "update":
                parts.append("Update todo → todos.")
            else:
                parts.append("Delete todo → todos.")
